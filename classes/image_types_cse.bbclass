...
generate_imx_sdcard () {
	...
	kk_bin="`readlink -f ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${MACHINE}.bin`"
	kk_dtb="`readlink -f ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${UBOOT_DTB_NAME}`"
	kk_img="`readlink -f ${DEPLOY_DIR_IMAGE}/uramdisk-recovery.img`"

	dd if=${DEPLOY_DIR_IMAGE}/imx-boot-${MACHINE}-${UBOOT_CONFIG}.bin of=${SDCARD} conv=notrunc seek=${IMX_BOOT_SEEK} bs=1K
	dd if=${kk_dtb} of=${SDCARD} conv=notrunc seek=5120 bs=512
	dd if=${kk_bin} of=${SDCARD} conv=notrunc seek=6144 bs=512
	dd if=${kk_img} of=${SDCARD} conv=notrunc seek=57344 bs=512
