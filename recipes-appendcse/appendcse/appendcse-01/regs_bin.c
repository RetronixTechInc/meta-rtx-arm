...
void run_cmd(int argc, char *argv[])
{
	...
	do {
		...
		sscanf(argv[2], "%s", cmd);
		for (iCount = 0 ; iCount < ( argc - 3 ) ; iCount ++)
		{
			sscanf(argv[iCount+3], "%x", (unsigned int *) &num[iCount]);
		}
		count = argc - 2 ;
...
//		printf("%s <dev>\n", argv[0]);
		printf("Version: 0.0.1 2021/4/1\n");
		printf("Usage:\n");
		printf("  %s <dev> r <register> [count]\n", argv[0]);
		printf("  %s <dev> w <register> value1 [value2 value3 ...]\n", argv[0]);
		printf("Example:\n");
		printf("  %s spi2.0 r 1101\n", argv[0]);
		printf("  %s spi2.0 w 302 00 11 22 33 44 55\n", argv[0]);
