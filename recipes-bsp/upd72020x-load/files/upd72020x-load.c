...
void usage() {
    ...
    printf("usage: upd72020 -r -m domian -b bus -d dev -f fct -s -o outfile : read eeprom to file (size default is 0x10000 or 64KB)\n");
    //printf("usage: upd7202 -c -b -d -f -s -i outfile : check eeprom against file\n");
    printf("usage: upd72020 -w -m domian -b bus -d dev -f fct -i infile : write file to eeprom\n");
    printf("usage: upd72020 -u -m domian -b bus -d dev -f fct -i infile : upload file to firmware memory\n");
}
...
int main(int argc, char **argv) {
    ...
    uint32_t domain, bus, dev, fct;
    uint32_t size = 0x10000;
    uint32_t rflag = 0;
    uint32_t wflag = 0;
    uint32_t uflag = 0;
    uint32_t mflag, bflag, dflag, fflag, sflag, fileflag = 0;
    char *filename = NULL;
    char pcicfgfile[100];
    int c;
    opterr = 0;
    ...
    while ((c = getopt(argc, argv, "rwub:m:d:f:o:i:l:s:")) != -1) {
        switch (c) {
            case 'r':
            ...
            case 'm':
                mflag = 1;
                domain = strtoul(optarg, NULL, 16); //hex numbers for size!!!
                break;
            case 'b':
                bflag = 1;
